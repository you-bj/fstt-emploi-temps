...
class ScheduleGenerator:
    ...
    # Setting to 15h/week to ensure all groups get full schedules
    MAX_TEACHER_HOURS_PER_WEEK = 15
    
    # Maximum hours per day for teachers (5 sessions × 1.5h = 7.5h)
    MAX_TEACHER_HOURS_PER_DAY = 7.5
    
    # ═══════════════════════════════════════════════════════════
    # STUDENT CONSTRAINTS
    # Students should study 18 hours per week (6 Cours + 6 TD × 1.5h each)
    # 18h/week ÷ 5 days = 3.6h/day average, allow up to 6h/day for flexibility
    # ═══════════════════════════════════════════════════════════
    MAX_STUDENT_HOURS_PER_WEEK = 18  # 6 Cours + 6 TD = 12 sessions × 1.5h
    MAX_STUDENT_HOURS_PER_DAY = 6    # Maximum 4 sessions per day
    ...
    def generate_schedule_for_group(self, groupe_id: int, matiere: str,
        ...
        if module_departement:
            try:
                ...
                if not is_valid:
                    # Just log warning — teacher was already selected by find_suitable_teacher_for_module
                    # which handles specialty matching with fallback. Don't block here.
                    pass
            except AttributeError:
                # If method doesn't exist, skip this validation
                pass
        
        # Get teacher specific max daily hours
        max_minutes = self.db.get_duree_max_enseignant(enseignant_id)
        teacher_max_daily_hours = max_minutes / 60.0 if max_minutes else self.MAX_TEACHER_HOURS_PER_DAY
        ...
        # Get available rooms with sufficient capacity, preferring matching type
        available_rooms = self._get_suitable_rooms(effectif, type_seance)
        ...
        for date in dates:
            ...
            if current_daily_hours + duree_heures > teacher_max_daily_hours:
    ...
    def _get_suitable_rooms(self, min_capacity: int, type_seance: str = "") -> List[Dict]:
        """Gets rooms with sufficient capacity, preferring rooms matching session type"""
        try:
            conn = self.db.get_connection()
            cursor = conn.cursor()
            cursor.execute(
            ...
            # Room type matching: Cours→Amphithéâtre first, TD→Salle first, TP→Laboratoire first
            # Preferred rooms tried first, fallback rooms used when preferred are fully booked
            if type_seance:
                preferred = []
                fallback = []
                ts_lower = type_seance.lower()
                for room in rooms:
                    rtype = (room.get('type_salle') or '').lower()
                    if ts_lower in ('tp', 'travaux pratiques'):
                        if rtype == 'laboratoire':
                            preferred.append(room)
                        else:
                            fallback.append(room)
                    elif ts_lower in ('cours', 'cours magistral'):
                        if rtype in ('amphithéâtre', 'amphitheatre'):
                            preferred.append(room)
                        else:
                            fallback.append(room)
                    elif ts_lower in ('td', 'travaux dirigés', 'travaux diriges'):
                        if rtype == 'salle':
                            preferred.append(room)
                        else:
                            fallback.append(room)
                        ...
                        fallback.append(room)
                # Preferred rooms first, fallback after — generator tries each in order
    ...
    def find_suitable_teacher_for_module(self, departement: str, 
        ...
        Picks the least-loaded teacher. Falls back to any available teacher if no specialty match.
        """
        if teacher_weekly_hours is None:
            teacher_weekly_hours = {}
        ...
        # Filter to teachers with available hours, pick least-loaded
        available_teachers = []
        for teacher in teachers:
            ...
            if current_hours < self.MAX_TEACHER_HOURS_PER_WEEK:
                available_teachers.append((teacher_id, current_hours))
        
        if available_teachers:
            # Pick the least-loaded teacher for even distribution
            available_teachers.sort(key=lambda x: x[1])
            return available_teachers[0][0]
        
        # FALLBACK: no specialty-matched teacher available → use ANY available teacher
        all_teachers = self.db.get_tous_utilisateurs('enseignant')
        fallback = []
        for t in all_teachers:
            tid = t[0]
            if tid in excluded_teacher_ids:
                continue
            current_hours = teacher_weekly_hours.get(tid, 0.0)
            if current_hours < self.MAX_TEACHER_HOURS_PER_WEEK:
                fallback.append((tid, current_hours))
